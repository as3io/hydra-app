...
4. The server is now accessible on `http://localhost:8405/` (or whatever port you configure)
...
```
 **Note:** If you are not running on OSX, or you have customized the [cygnusb2b/hydra-engine](https://github.com/cygnusb2b/hydra-engine) port, you will need to customize the `EMBER_GRAPH_PROXY` URL to point to the IP/Hostname and port of your graph instance:
...
To re-initialize your entire environment, execute `yarn run stop` to shutdown. Then run `docker volume rm hydraapp_node_modules` to remove the cached dependancies. Finally, execute `docker-compose -p hydraapp rebuild` to force rebuilding the application from the project `Dockerfile` (Typically only needed when making changes to the `docker-compose.yml` configuration.) Executing `yarn run start` again will re-initialize and start up the environment from scratch.
...
The test environment can be booted and run by executing `yarn run test` or `yarn run coverage`, or manually via `docker-compose -p hydraapptest -f test/docker-compose.yml run --entrypoint "yarn run test" test`.
